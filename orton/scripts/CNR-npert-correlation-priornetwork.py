...
# Use a prior network
PRIOR_NETWORK = []
for source, targets in REF_IMAP.items():
    for target in targets[targets == 1].index:
        PRIOR_NETWORK.append((target, source))
...
for NPERT in NPERT_VALS:
    ...
    for rep in range(NREP):
        ...
        p = cnr.CnrProblem(panel_braf_wt, eta=eta/2, theta=theta/2,
                           bounds=BOUNDS, prior_network=PRIOR_NETWORK)
        p.cpx.parameters.threads.set(24)
        p.cpx.solve()
        if p.cpx.solution.is_primal_feasible():
        ...
        p = cnr.CnrProblem(panel_ras_wt, eta=eta/2, theta=theta/2,
                           bounds=BOUNDS, prior_network=PRIOR_NETWORK)
        p.cpx.parameters.threads.set(24)
        p.cpx.solve()
        if p.cpx.solution.is_primal_feasible():
        ...
        p = cnr.CnrProblem(panel_egfr_wt, eta=eta/2, theta=theta/2,
                           bounds=BOUNDS, prior_network=PRIOR_NETWORK)
        p.cpx.parameters.threads.set(24)
        p.cpx.solve()
        if p.cpx.solution.is_primal_feasible():
...
with open("../results/solutions/sols_npert_correlations-priornetwork.pickle", "wb") as handle:
